...
def data_collection(consolidated_array, time_range, trade_outcome):
    ...
    vari = np.array([], dtype=np.float64)
    ...
    for time in time_range:
        ...
        vari = np.append(vari, np.var(consolidated_array[:, -time:]))
        # diff = np.append(diff, np.sum(np.diff(consolidated_array[:, -time:])))

    # Consolidate
    data_point = np.array([stdev, vari], dtype=np.float64).flatten()
    data_point = np.append(data_point, [trade_outcome])
...
def training():
    ...
    data = np.load(pr.data_store_location+'mlp_svc_input.npy', allow_pickle=True)
    cols_to_delete = list(range(0,58))
    cols_to_delete.insert(-1, -1)
    ...
    X_trg = data[:round(trg_percent*rows),:-1]
    Y_trg = np.int32(data[:round(trg_percent*rows),-1:])
    X_test = data[round((trg_percent)*rows):, :-1]
    Y_test = np.int32(data[round((trg_percent)*rows):, -1:])

    # clf = make_pipeline(StandardScaler(), SVC(C=0.01, probability=False, kernel='linear'))
    # clf = SVC(C=1e-8, probability=True, kernel='linear', gamma='scale', random_state=1)
    # clf = make_pipeline(StandardScaler(),  MLPClassifier(solver='adam', alpha=0.5, hidden_layer_sizes=(2000), random_state=1)) # https://is.gd/vOasOc
    # clf = tree.DecisionTreeClassifier()
    clf = make_pipeline(StandardScaler(), SGDClassifier(loss="hinge", penalty="elasticnet", max_iter=200))
    clf.fit(X_trg,Y_trg)
...
def svc_trading(consolidated_array): #, action, predicted_delta, test_nrmse, delay , test_range_center, lookback):
    ...
    # Consolidate
    data_point = np.array([vari], dtype=np.float64).flatten()
